  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
        <StatCard label="Recaudado este Mes (Bs.)" value={formatVES(Number(summary?.month_collected_ves ?? 0))} colorClass="text-green-600" />
        <StatCard label="Tasa Actual (Bs./USD)" value={formatVES(Number(latest?.rate ?? 0))} colorClass="text-blue-600" subtitle={latest?.effective_date} />
        <StatCard label="Pagos este Mes" value={formatDashboardValue(summary?.month_payment_count)} />
      </div>
      {chartData.length > 0 && (
        <div className="card">
